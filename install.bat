...
where adb >nul 2>&1
if %errorlevel% neq 0 (
    echo [31mERROR: adb not found. Install Android SDK Platform Tools.[0m
    exit /b 1
)
...
if exist "%ADBKEY%" if exist "%ADBKEY_PUB%" (
    echo [36m==^> Pushing ADB keys ^(for force-stop ^& split-screen^)...[0m
    %ADB% push "%ADBKEY%" /data/local/tmp/adbkey
    %ADB% push "%ADBKEY_PUB%" /data/local/tmp/adbkey.pub
    REM run-as only works for debuggable builds; try it but don't fail
    %ADB% shell "run-as %PACKAGE% mkdir -p ./files" >nul 2>&1
    if not errorlevel 1 (
        %ADB% shell "run-as %PACKAGE% cp /data/local/tmp/adbkey ./files/adbkey" >nul 2>&1
        %ADB% shell "run-as %PACKAGE% cp /data/local/tmp/adbkey.pub ./files/adbkey.pub" >nul 2>&1
        echo [32m    ADB keys installed.[0m
    ) else (
        echo [32m    ADB keys pushed to /data/local/tmp/[0m
        echo [36m    The app will pick them up from there on first launch.[0m
    )
) else (
